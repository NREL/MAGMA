  ...
  if ( typeof(total.generation)=='character' ) {
    ...
    if ( typeof(r.z.gen)=='character' ) {
      ...
      p1 <- gen_stack_plot(r.z.gen[(!Region %in% ignore.regions),],
                           ...
                           filters = c('Region'))
      print(p1[[1]] + facet_wrap(~Region, ncol=wrap.cols) + 
        theme(aspect.ratio = 2.5/length(unique(r.z.gen$scenario)), axis.text.x = element_text(angle = -30, hjust = 0),
            text = element_text(face='bold')))
...
***
## 12.2 Total Gen by Region GWh
```{r annual-generation-table-region, fig.width=10}
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if (annual.generation.table){
  if ( typeof(total.generation)=='character' | typeof(total.avail.cap) == 'character' ) {
    print('INPUT ERROR: total.generation or total.avail.cap has errors. Cannot run this section.')
    ...
    # Set the order level that generatino type will be displayed.
    r.z.gen[, Type := factor(Type, levels = c(gen.order))] 
    setorder(r.z.gen,Type)
    
    r.z.gen = r.z.gen[Type!='Curtailment', ]
    
    # Calculate the percent generation of each type
    r.z.gen[, Total:=sum(GWh), by=.(scenario,Region)]
    r.z.gen[, Percent:=GWh/Total*100]
    r.z.gen[, Total:=NULL]

    # recast so scenarios are columns
    r.z.gen.val = dcast(r.z.gen, Type+Region~scenario, value.var='GWh')
    
    kable(r.z.gen.val[order(Region)], format.args = list(big.mark = ','), digits=2) 
...
#### Percent of Total Generation by Region
```{r annual-percent-generation-table-region, fig.width=10}
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if (annual.generation.table){
  if ( typeof(total.generation)=='character' | typeof(total.avail.cap) == 'character' ) {
    print('INPUT ERROR: total.generation or total.avail.cap has errors. Cannot run this section.')
    ...
    # recast so scenarios are columns
    r.z.gen.percent = dcast(r.z.gen, Type+Region~scenario, value.var='Percent')

    kable(r.z.gen.percent[order(Region)], format.args = list(big.mark = ','), digits=2) 
...
if(installed.cap.plot) {
  ...
  # if ( typeof(total.rated.cap)=='character' ) {
  #   print('INPUT ERROR: total.rated.cap has errors. Cannot run this section.')
  # } else{
  #   # Add type
  #   total.rated.cap[, Type:=gen.type.mapping[as.character(name)] ]
  #   total.rated.cap[, GWh:=value] # Changing this so we can use gen_stack_plot
  #   # Create plot
  #   plot.data = gen_stack_plot(total.rated.cap)
  #   print(plot.data[[1]] + scale_y_continuous(breaks=plot.data[[2]], limits=c(min(plot.data[[2]]), max(plot.data[[2]])), 
  #                            expand=c(0,0), label=comma) + 
  #           ylab("Rated Capacity, GW") +
  #           theme(aspect.ratio = 2.0, axis.text.x = element_text(angle = -30, hjust = 0),
  #           text = element_text(face='bold')))
  # }
...
***
## 37. Line Violation Table
```{r line-violation-table}
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# Check if this section was selected to run in the input file
if(line.violation.table) {
  if (length(lines)>0) {
    if (typeof(total.line.violation)=='character'){
      print('INPUT ERROR: total.line.flow has errors. Cannot run this section')
    } else{
      # Call the query function to get line flows for the lines selected in the input file.
      line.violations = tryCatch( annual_line_violation(total.line.violation), 
                             error=function(cond) {return('ERROR: line_flows query not returning correct results.')})
      
      # Print data 
      line.violations.sum<-line.violations[,.(`Abs Val GWh`=sum(GWh)),by=.(scenario)]
      kable(line.violations.sum, digits=1)
...
***
## 37. Line Violation Histogram
```{r line-violation-hist}
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# Check if this section was selected to run in the input file
if(line.violation.table) {
  if (length(lines)>0) {
    if (typeof(total.line.violation)=='character'){
      ...
      # Print histogram
      ggplot(line.violations[GWh>0.1,])+geom_histogram(aes(GWh,fill=scenario),binwidth=1,position="dodge")+ylab('Number of lines')+xlab('Absolute value of annual violation GWh (excluding <0.1)')+
...
***
## 37. Transformer Violation Table
```{r trans-violation-table}
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# Check if this section was selected to run in the input file
if(line.violation.table) {
  if (length(lines)>0) {
    if (typeof(total.trans.violation)=='character'){
      print('INPUT ERROR: total.trans.flow has errors. Cannot run this section')
    } else{
      # Call the query function to get line flows for the lines selected in the input file.
      trans.violations = tryCatch( annual_trans_violation(total.trans.violation), 
                             error=function(cond) {return('ERROR: annual_trans_violation query not returning correct results.')})
      
      # Print data 
     trans.violations.sum<-trans.violations[,.(`Abs Val GWh`=sum(GWh)),by=.(scenario)]
      kable(trans.violations.sum, digits=1)
...
***
## 37. Transformer Violation Histogram
```{r trans-violation-hist}
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# Check if this section was selected to run in the input file
if(line.violation.table) {
  if (length(lines)>0) {
    if (typeof(total.trans.violation)=='character'){
      ...
      # Print histogram
      ggplot(trans.violations[GWh>0.1,])+geom_histogram(aes(GWh,fill=scenario),binwidth=1,position="dodge")+ylab('Number of lines')+xlab('Absolute value of annual violation GWh (excluding <0.1)')+
        theme(legend.key = element_rect(color = "grey70", size = 0.8),
               legend.key.size = grid::unit(1.5, "lines"),
               legend.text = element_text(size=text.plot,face = 'bold'),
               text=element_text(size=text.plot,face = 'bold'),
...
***
## 38. Line Loading Histogram
```{r line-loading-hist}
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# Check if this section was selected to run in the input file
if(line.loading.table) {
  if (length(lines)>0) {
    if (typeof(total.line.loading)=='character'){
      print('INPUT ERROR: total.line.loading has errors. Cannot run this section')
    } else{
      # Call the query function to get line flows for the lines selected in the input file.
      line.loading = tryCatch( avg_line_loading(total.line.loading), 
                             error=function(cond) {return('ERROR: avg_line_loading query not returning correct results.')})
      
      # Print histogram
      ggplot(line.loading[Loading>0.0,])+geom_histogram(aes(Loading,fill=scenario),binwidth=5,position="dodge")+ylab('Number of lines')+xlab('Average Loading % (bin width 5%)')+
        theme(legend.key = element_rect(color = "grey70", size = 0.8),
               legend.key.size = grid::unit(1.5, "lines"),
               legend.text = element_text(size=text.plot,face = 'bold'),
               text=element_text(size=text.plot,face = 'bold'),
...
***
## 38. Transformer Loading Histogram
```{r trans-loading-hist}
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# Check if this section was selected to run in the input file
if(line.loading.table) {
  if (length(lines)>0) {
    if (typeof(total.trans.loading)=='character'){
      print('INPUT ERROR: total.line.loading has errors. Cannot run this section')
    } else{
      # Call the query function to get line flows for the lines selected in the input file.
      trans.loading = tryCatch( avg_trans_loading(total.trans.loading), 
                             error=function(cond) {return('ERROR: avg_trans_loading query not returning correct results.')})
      
      # Print histogram
      ggplot(trans.loading[Loading>0.0,])+geom_histogram(aes(Loading,fill=scenario),binwidth=5,position="dodge")+ylab('Number of transformers')+xlab('Average Loading % (bin width 5%)')+
        theme(legend.key = element_rect(color = "grey70", size = 0.8),
               legend.key.size = grid::unit(1.5, "lines"),
               legend.text = element_text(size=text.plot,face = 'bold'),
               text=element_text(size=text.plot,face = 'bold'),
