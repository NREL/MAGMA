...
if ( 11 %in% run.sections | 12 %in% run.sections | 13 %in% run.sections ) {
...
if ( typeof(r.z.gen.plot)=='character' ) {
  ...
  print('ERROR: region_load function not returning correct results.')
    ...
    region.names = unique(r.z.gen.plot$Region) # List of all regions to make plots for
...
***
## 5. Specified Period Dispatch Stacks
  ...
  for ( p in 1:n.periods ) {
    for ( z in 1:length(zone.names)) ) {
      for ( r in 1:length(region.names)) ) {
        ...
        key.plots[[idx]] = knit_expand(file = 'source_scripts/p_key_period_dispatch.Rmd', period.name = period.name, region.name = region.name, zone.name = zone.name)
        idx = idx+1
      }
    }
  }
...
***
### 7. Average Interval Curtailment
```{r daily-curtailment}
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
***
## 13. Specified Period Interface Flow Plot
```{r zonal-flow-plots}
...
## Reserves
***
### 14. Annual Reserve Provisions and Shortage
...
***
### 15. Average Interval and Daily Reserve Provisions
```{r reserves-plots}
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if (15 %in% run.sections) {
...
***
## 16. Region and Zone Generation (GWh)
