...
if (exists('region.names')) { g.height = length(region.names[!region.names%in%ignore.regions]) * 5
  } else {g.height = 5 }
...
if (total.gen.stack){
  if ( typeof(total.generation)=='character' ) {
    ...
    print(plot.data[[1]] + theme(aspect.ratio = 2.5/length(unique(yr.gen$scenario)),axis.text.x = element_text(angle = -30, hjust = 0)) +
...
if (zone.gen.stacks) {
  if ( typeof(total.generation)=='character' ) {
    ...
    print(plot.data[[1]] + facet_wrap(~Zone, scales = 'free', ncol=)+
