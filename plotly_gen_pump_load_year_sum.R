...
plotly_3d_gen_pumpload <- function(DR_data, agg.style = "Total", date_focus = NA, hour_focus = NA, 
  ...
  file.loc.zone.prop.scen <- lapply(DR_data_focus_scen, function(scen.., zone.. = DR_data_zones, DR_data_focus.. = DR_data, prop.. = DR_data_properties, sd.. = sd..., pn.. = period_name){
    
    file.loc.zone.prop <- lapply(prop.., function(prop., scen. = scen.., zone. = zone.., DR_data_focus. = DR_data_focus.., sd. = sd.., pn. = pn..){
      
      file.loc.zone <- lapply(zone., function(zone, prop = prop., scen = scen., DR_data_focus = DR_data_focus., sd = sd., pn = pn.){
        
        DR_data_focus <- DR_data_focus[property == prop & scenario == scen & Location == zone,]
        ...
        p <- plot_ly(x = dates, 
          ...
          layout(scene = list(xaxis = list(title = 'Day', tickformat = "%b-%d"),
                 ...
                 title = paste0(scen, ": ", prop))
        
        saveWidgetFix(p, file = file.path(sd, paste0(paste(pn, zone, scen, prop, sep = "_"),".html")),
                      title = paste(pn, zone, scen, prop, sep = "_"), 
                      selfcontained = T)
        
        return(file.path(sd, paste0(paste(pn, zone, scen, prop, sep = "_"),".html")))
        
      })
      
      names(file.loc.zone) <- zone.
      return(file.loc.zone)
      
    })
    
    names(file.loc.zone.prop) <- prop..
    return(file.loc.zone.prop)
    
  })
  
  names(file.loc.zone.prop.scen) <- DR_data_focus_scen
  return(file.loc.zone.prop.scen)
