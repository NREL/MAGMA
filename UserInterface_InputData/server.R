...
shinyServer(function(input, output) {
  ...
  observeEvent(input$createCSV, {
    ...
    write_data = list( inputDatabase.Location = input$db.loc,
                            ...
                            DA.RT.Plot.Types = c( input$rDA_RT_Type1,  input$rDA_RT_Type2,  input$rDA_RT_Type3,  input$rDA_RT_Type4,  input$rDA_RT_Type5, 
                                                  input$rDA_RT_Type6,  input$rDA_RT_Type7,  input$rDA_RT_Type8,  input$rDA_RT_Type9,  input$rDA_RT_Type10, 
                                                  input$rDA_RT_Type11, input$rDA_RT_Type12, input$rDA_RT_Type13, input$rDA_RT_Type14, input$rDA_RT_Type15, 
                                                  input$rDA_RT_Type16, input$rDA_RT_Type17, input$rDA_RT_Type18, input$rDA_RT_Type19, input$rDA_RT_Type20 ),	
    ...
    write_data_df = as.data.frame(stri_list2matrix(write_data))
    names(write_data_df) = names(write_data)
    
    write_data_df$Plot.Color[which(write_data_df$Gen.Type=="")]=NULL
