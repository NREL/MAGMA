...
dr_gen_pump_load_data_comp <- function(DR_categories, generation_data = interval.generation, 
  ...
  if(aggregation == "Total"){
    ...
    if(!keep_category){
      ...
      generation_data_DR <- generation_data_DR[, .(value = -sum(value)/1000), 
                                               by = c("property", "time", "scenario")]
      pumped_load_data_DR <- pumped_load_data_DR[, .(value = sum(value)/1000), 
                                                 by = c("property", "time", "scenario")]
    ...
    if(!keep_category){
      ...
      generation_data_DR <- 
        generation_data_DR[, .(value = -sum(value)/1000), 
                           by = c("property", "time", "scenario", 
                                  c("Region", "Zone")[match(aggregation, c("Regional", "Zonal"))])]
      
      pumped_load_data_DR <- 
        pumped_load_data_DR[, .(value = sum(value)/1000), 
                            by = c("property", "time", "scenario", 
                                   c("Region", "Zone")[match(aggregation, c("Regional", "Zonal"))])]
...
dr_gen_pump_load_by_keyperiod <- function(comb.gen.pump.data, aggregation = "Total", facet_by_location = FALSE, 
  ...
  p <- ggplot(comb.gen.pump.data)+
    ...
    scale_color_manual(values = color_scheme,labels=c('Load Reduction','Load Increase'))+
    
    scale_y_continuous(name = "Load Shifted (GWh)\n")+
    
    theme(text = element_text(size=15),
#added to fix font size
...
dr_gen_pump_load_by_dispatch <- function(total_data_DR, aggregation = "Total",
  ...
  p <- ggplot(total_data_DR)+
    ...
    theme(text = element_text(size=15),
          axis.text.x = element_text(angle = x_axis_angle, hjust = .5),
          axis.title.x = element_blank())+
