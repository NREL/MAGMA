...
knitr::opts_chunk$set(echo=FALSE, comment=NA, warning=FALSE, message=FALSE, include=TRUE,
                      fig.path=fig.path.name, dev='png', fig.width=12, cache=FALSE)
    ...
    if ( typeof(key.period.gen)=='character' ) {
      ...
      for ( p in 1:n.periods ) {
        ...
        if (key.period.dispatch.total.log) {
          key.plots.total[[p]] = knit_expand(file = 'source_scripts/c_p_key_period_dispatch_total.Rmd', period.name = period.name)
        ...
        if (key.period.dispatch.zone.log) {
          for ( z in 1:length(zone.names) ) {
            ...
            key.plots.zone[[z.idx]] = knit_expand(file = 'source_scripts/c_p_key_period_dispatch_zone.Rmd', period.name = period.name, zone.name = zone.name) 
            z.idx = z.idx + 1
          } 
        ...
        if (key.period.dispatch.region.log) {
          for ( r in 1:length(region.names) ) {
            ...
            key.plots.region[[r.idx]] = knit_expand(file = 'source_scripts/c_p_key_period_dispatch_region.Rmd', period.name = period.name, region.name = region.name)
            r.idx = r.idx + 1
          }
