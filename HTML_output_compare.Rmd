...
if (exists('zone.names')) { z.height = ( length(zone.names[!zone.names%in%ignore.zones]) %/% 3 + 
                                           length(zone.names[!zone.names%in%ignore.zones]) %% 3 ) * 5
  } else { z.height = 5 }
...
s.height = ( length(db.loc) ) * 4.5
p.height = ( length(period.names) %/% 2 + length(period.names) %% 2 ) * 5
st.height = ( length(db.loc) * length(re.types) ) * 4
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
```
...
if (zone.gen.stacks) {
  if ( typeof(total.generation)=='character' ) {
    ...
    print(plot.data[[1]] + facet_wrap(~Zone, ncol=3)+theme(aspect.ratio = 2.5))
  }
} else { print('Section not run according to input file.') }
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
```
...
```{r region-gen-diff-stacks, fig.height=r.height, fig.width=10}
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
```{r yearly-curtailment-by-type, fig.height=st.height}
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
```{r daily-curtailment-by-type, fig.height=st.height}
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  if (length(interfaces)>0) {
    if ( typeof(interval.interface.flow) == 'character' ) {
      ...
      if ( typeof(interface.flows)=='character' ) { 
        ...
        # Get interval plots
        avg.interface = interface.flows[, .(value=sum(value)),by=.(scenario,name,interval)]
        ...
        avg.interface[, time := as.POSIXct(strptime(paste(hour,minute,second, sep=":"), "%H:%M:%S"),'UTC')]
      
        p1 = interface_plot(avg.interface, x_col = 'time', color='scenario')
        print(p1 + labs(title='Interval Flow') + facet_wrap(~name,ncol=1,scales='free') +
...
```{r price-duration-curve, fig.width = 12, fig.height=r.height}
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if ( typeof(compare.plot.data)=='character' ) {
      ...
      if (compare.dispatch.zone) {
        comp.gen.zone = mapply(function(t,z){ knit_expand(file = 'source_scripts/compare/c_p_key_period_gen_comparison_zone.Rmd', 
                               ...
                               rep(da.rt.types,each=length(zone.names)), rep(zone.names,each=length(da.rt.types)))
