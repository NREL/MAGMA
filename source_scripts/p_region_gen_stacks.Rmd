...
# Check if zonal gen exists. If not, return an error.
if ( exists('zone.gen') ) {

# Check if zonal.gen query worked and create plot of regional gen, else return an error.
if ( typeof(zone.gen)=='character' ) {
  print('ERROR: region_zone_gen function in "zone-gen-stacks" chunk did not return correct results.')
} else {
  ...
  region.load = join(region.load, zone.gen[,c('Zone', 'Region')], by = 'Region', type='left', match='first')
...
} else { print('Zonal generation not found. Make sure "zone-gen-stacks" chunk is running.') }
```
