...
```{r Gen-compare-zone-{{zone.name}}-{{type.name}}, results='asis', include=TRUE}
z.name = '{{zone.name}}' # Assign zone name
t.name = '{{type.name}}' # Assign generator type name
cat("### ", t.name, ': ', z.name) # Print period and zone name above the plot
text.plot=16 # Plot text size.
figure.width = 15 
figure.height = p.height
```

```{r Gen-compare-plot-zone-{{zone.name}}-{{type.name}}, fig.width=figure.width, fig.height=figure.height, include=TRUE}
# Check to make sure this zone shouldn't be ignored.
if ( !z.name %in% ignore.zones ) {
  
  # Select data for specific type and region
  plot.data = compare.plot.data[Type==t.name & Zone==z.name, 
                                .(GWh = sum(value)/1000), 
                                by=.(scenario, time, Zone, Type, Period)]
